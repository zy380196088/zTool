  ...
  */
  ...
  ok( pass, "jQuery('&lt;tag&gt;') needs optional document parameter to ease cross-frame DOM wrangling, see #968" );*/
	...
	"selector state": function (test) {
		...
		var d = "<div />";
		test.equals(jQuery(d).appendTo(jQuery(d)).selector, jQuery(d).appendTo(d).selector, "manipulation methods make same selector for jQuery objects");
		test.done();
	},
	...
	"toArray()": function (test) {
		test.expect(1);
		test.same(jQuery("#main p").toArray(), q("firstp", "ap", "sndp", "en", "sap", "first"), "Convert jQuery object to an Array");
		test.done();
	},
	...
	"map()": function (test) {
		...
		test.same(jQuery("#ap").map(function () {
			return jQuery(this).find("a").get();
		}).get(), q("google", "groups", "anchor1", "mark"), "Array Map");

		test.same(jQuery("#ap > a").map(function () {
	...
	"jQuery.makeArray": function (test) {
		...
		test.equals(jQuery.makeArray(function () {
			return 1;
		...
		jQuery.each(selectors, function (i, selector) {
			...
			jQuery.each(methods, function () {
				...
				jQuery.each(contexts, function (i, context) {
					...
					test.same(jQuery(selector, context)[method](arg).subclassMethod, undefined, 'jQuery' + description + ' doesnt have Subclass methods');
					test.same(jQuery(selector, context)[method](arg).subclassSubclassMethod, undefined, 'jQuery' + description + ' doesnt have SubclassSubclass methods');
					test.same(Subclass(selector, context)[method](arg).subclassMethod, Subclass.fn.subclassMethod, 'Subclass' + description + ' has Subclass methods');
					test.same(Subclass(selector, context)[method](arg).subclassSubclassMethod, undefined, 'Subclass' + description + ' doesnt have SubclassSubclass methods');
					test.same(SubclassSubclass(selector, context)[method](arg).subclassMethod, Subclass.fn.subclassMethod, 'SubclassSubclass' + description + ' has Subclass methods');
					test.same(SubclassSubclass(selector, context)[method](arg).subclassSubclassMethod, SubclassSubclass.fn.subclassSubclassMethod, 'SubclassSubclass' + description + ' has SubclassSubclass methods');
				});
			});
		});
		test.done();
